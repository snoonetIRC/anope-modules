...
struct ServerNewsItem : Serializable
{
	Anope::string text;
	Anope::string who;
	Anope::string server;
	...
	ServerNewsItem() : Serializable("ServerNewsItem") { }
};

class ServerNewsService : public Service
{
 public:
	ServerNewsService(Module *m) : Service(m, "ServerNewsService", "server_news") { }
...
struct MyNewsItem : ServerNewsItem
{
...
class MyNewsService : public ServerNewsService
{
	NewsItems newsItems;
 public:
	MyNewsService(Module *m) : ServerNewsService(m) { }
...
class CommandOSServerNews : public Command
{
 ...
 protected:
	void DoList(CommandSource &source)
	{
		...
		else
		{
			...
			lflist.AddColumn(_("Number")).AddColumn(_("Creator")).AddColumn(_("Created")).AddColumn(_("Server")).AddColumn(_("Text"));
 ...
 public:
	CommandOSServerNews(Module *creator) : Command(creator, "operserv/servernews", 1, 3), ns(news_service)
	{
