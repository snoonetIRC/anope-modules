...
class HSRegHost : public Module {
 private:
    ...
    Anope::string CleanNick(Anope::string &nick)
    {
        ...
        const Anope::string &badStartChars = Config->GetBlock("networkinfo")->Get<const Anope::string>("disallow_start_or_end");
        const Anope::string &vhostChars = Config->GetBlock("networkinfo")->Get<const Anope::string>("vhost_chars");

        if (badStartChars.find_first_of(nick[0]) != std::string::npos)
            nick[0] = '-';
        ...
        return nick != oldNick ? HashNick(oldNick) : "";
    }
